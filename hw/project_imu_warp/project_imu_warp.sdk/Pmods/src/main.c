...
/*  File Description:                                                   */
/*                                                                      */
/*  This file contains a demo to be used with the Pmod NAV              */
/*	Accelerometer, Magnetometer, Temperature, and Barometric Pressure	*/
/*	data is captured over a SPI interface and displayed over UART		*/
/*																		*/
...
/*  Problems:                                                           */
/*                                                                      */
/*  In order to include "math.h" you must include m in compiler         */
/*  settings see https://www.xilinx.com/support/answers/52971.html      */
/*                                                                      */
/************************************************************************/
...
**
**  Description:
**		This function initializes the hardware used in the demo and starts a PmodNAV driver device
**
*/
void NavDemo_Initialize(void) {
...
**
**  Description:
**		This function encapsulates the PmodNAV demo, capturing data from each part on the PmodNAV and
**		displaying it over a UART connection
**
*/
void NavDemo_Run(void) {
    float Pref,
    ...
    while(1){
        ...
        printf("Device IDs\n\r");
        printf("A/G ID:%d  MAG ID:%d  ALT ID:%d\n\r\n\r", nav.idData.ag, nav.idData.mag, nav.idData.alt);
        ...
        //format and serial print the data read from Gyro instrument, stored in gyroData variables and expressed in degrees per second

        printf("Magnetometer Gauss values   X:%.2f Gauss, Y:%.2f Gauss, Z:%.2f Gauss\n\r",
			nav.magData.X,
			nav.magData.Y,
			nav.magData.Z
        );
        ...
        str = compass[(int)((magXYd + 22.5) / 45.0) % 8]; // get appropriate compass string - north is centered on 0 degrees
...
**
**   Return Values:
**      float altMeters - it returns the current altitude based on the measured pressure and the previously computed reference pressure
**
**   Errors:
**		none
**
...
**
**  Return Value:
**      p - returns the polar coordinate representation of the vector r projected onto the XY plane
**
**  Errors:
**		none
**
...
**
**   Description:
**		This function enables the instruction and/or data caches on architectures that require them
**
*/
void NavDemo_EnableCaches(void)
{
...
**
**   Description:
**		This function disables the instruction and/or data caches on architectures that require them
**
*/
void NavDemo_DisableCaches(void)
{
